...
const CLAUDE_MODEL = 'claude-sonnet-4-5'; // swap to 'claude-sonnet-4-6' (newest) or 'claude-opus-4-6' (higher quality) if you have access
...
function generateICS(events, calendarName) {
  ...
  events.forEach(e => {
    ...
    lines.push('UID:' + e.id + '@nestly.app');
...
export default function App() {
  ...
  if (!data || !fontsReady) {
    return (
      <div style={{ background: '#F4F9FF', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: baseFont }}>
        <div style={{ fontFamily: displayFont, fontSize: 22, color: '#0C1A2E', fontStyle: 'italic', letterSpacing: '-0.01em' }}>Setting things up…</div>
      </div>
    );
  }
  ...
  return (
    <div style={{ background: '#F4F9FF', minHeight: '100vh', fontFamily: baseFont, color: '#0C1A2E', paddingBottom: 88 }}>
      ...
      {showSettings && <SettingsPanel data={data} setData={setData} onClose={() => setShowSettings(false)} displayFont={displayFont} />}
    </div>
  );
}
...
function Header({ familyName, onSettings, displayFont }) {
  ...
  return (
    <header style={{ borderBottom: '1px solid #CCE0F7', background: '#F4F9FF', position: 'sticky', top: 0, zIndex: 10, backdropFilter: 'blur(8px)' }}>
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '20px 20px 18px', display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
        <div>
          <div style={{ fontSize: 11, color: '#5B7A9D', letterSpacing: '0.16em', textTransform: 'uppercase', fontWeight: 500, marginBottom: 4 }}>{greeting}</div>
          <h1 style={{ fontFamily: displayFont, fontSize: 26, fontWeight: 500, letterSpacing: '-0.02em', lineHeight: 1.05, margin: 0, color: '#0C1A2E' }}>
            {familyName}
          </h1>
        </div>
        <button onClick={onSettings} style={{ background: 'transparent', border: '1px solid #B8D2F0', borderRadius: '50%', width: 38, height: 38, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#4A6485' }} aria-label="Settings">
          <Settings size={16} />
        </button>
      </div>
    </header>
...
function BottomNav({ view, setView }) {
  ...
  return (
    <nav style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: '#FFFFFF', borderTop: '1px solid #CCE0F7', padding: '10px 6px 14px', zIndex: 20 }}>
      <div style={{ maxWidth: 640, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 1 }}>
        {tabs.map(t => {
          const Icon = t.icon;
          const active = view === t.id;
          return (
            <button key={t.id} onClick={() => setView(t.id)} style={{ background: 'transparent', border: 'none', padding: '6px 4px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, cursor: 'pointer', color: active ? '#1558CE' : '#5B7A9D' }}>
...
function Onboarding({ onComplete, onDemo, displayFont, baseFont }) {
  ...
  return (
    <div style={{ background: '#F4F9FF', minHeight: '100vh', fontFamily: baseFont, color: '#0C1A2E' }}>
      <div style={{ maxWidth: 520, margin: '0 auto', padding: '60px 24px 40px' }}>
        <div style={{ fontSize: 11, color: '#1558CE', letterSpacing: '0.2em', textTransform: 'uppercase', fontWeight: 600, marginBottom: 18 }}>Nestly</div>
        <h1 style={{ fontFamily: displayFont, fontSize: 44, fontWeight: 500, letterSpacing: '-0.03em', lineHeight: 1.0, margin: '0 0 18px', color: '#0C1A2E' }}>
          The household runs<br /><em style={{ fontWeight: 400 }}>itself, almost.</em>
        </h1>
        <p style={{ fontSize: 16, lineHeight: 1.55, color: '#3A567A', margin: '0 0 36px', maxWidth: 440 }}>
        ...
        {step === 0 && (
          <>
            ...
            <div style={{ marginTop: 28, padding: '18px 0', borderTop: '1px solid #CCE0F7', fontSize: 13, color: '#5B7A9D', lineHeight: 1.5 }}>
              Data stays on your device. Designed for two-income families with kids under ten. <span style={{ color: '#1558CE', fontWeight: 600 }}>$49/month at launch.</span>
            </div>
          </>
        )}
        ...
        {step === 2 && (
          <div>
            ...
            <p style={{ fontSize: 13, color: '#5B7A9D', margin: '0 0 16px' }}>The more I know, the smarter I can be. Edit anytime.</p>
            {kids.map((k, i) => (
              <div key={k.id} style={{ background: '#FFFFFF', border: '1px solid #CCE0F7', borderRadius: 14, padding: 14, marginBottom: 12 }}>
                ...
                {kids.length > 1 && (
                  <button onClick={() => setKids(kids.filter((_, ix) => ix !== i))} style={{ background: 'transparent', border: 'none', color: '#5B7A9D', fontSize: 12, cursor: 'pointer', padding: '8px 0 0', display: 'flex', alignItems: 'center', gap: 4 }}>
                    <Trash2 size={12} /> Remove
                  </button>
                )}
              </div>
            ))}
            <button onClick={() => setKids([...kids, { id: 'k' + Date.now(), name: '', age: '', school: '', allergies: '', picky: '', interests: '', colour: KID_COLOURS[kids.length % KID_COLOURS.length].id }])} style={{ background: 'transparent', border: '1px dashed #B8D2F0', borderRadius: 12, padding: '10px 14px', cursor: 'pointer', color: '#4A6485', display: 'flex', alignItems: 'center', gap: 6, fontFamily: 'inherit', fontSize: 13, width: '100%', justifyContent: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 24 }}>
              ...
              <button onClick={finish} disabled={!kids.some(k => k.name.trim())} style={{ ...btnPrimary, opacity: kids.some(k => k.name.trim()) ? 1 : 0.4 }}>
                Enter Nestly <ChevronRight size={16} />
              </button>
            </div>
          </div>
        )}
...
function TodayView({ data, setData, displayFont }) {
  ...
  const generateDigest = async () => {
    ...
    try {
      ...
      const result = await callClaude("You are a warm, sharp family assistant. Brief and human. Never use emojis or exclamation marks.", prompt);
      setDigest(result);
    } catch (e) {
      setDigest("Couldn't reach the assistant right now. Try again in a moment.");
    }
  ...
  return (
    <div style={{ paddingTop: 24 }}>
      <div style={{ fontSize: 11, color: '#5B7A9D', letterSpacing: '0.16em', textTransform: 'uppercase', fontWeight: 500, marginBottom: 6 }}>{today}</div>
      ...
      {/* AI Digest card */}
      <div style={{ background: 'linear-gradient(135deg, #163E7E 0%, #0D2B6A 100%)', borderRadius: 18, padding: 20, marginBottom: 24, color: '#EDF4FF', position: 'relative', overflow: 'hidden' }}>
        ...
        {digest ? (
          ...
          <>
            ...
            <button onClick={generateDigest} disabled={digestLoading} style={{ background: '#EDF4FF', color: '#0D2B6A', border: 'none', borderRadius: 100, padding: '9px 16px', fontWeight: 600, fontSize: 13, fontFamily: 'inherit', cursor: digestLoading ? 'wait' : 'pointer', display: 'inline-flex', alignItems: 'center', gap: 6 }}>
      ...
      <div style={{ marginBottom: 8 }}>
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 12 }}>
          ...
          <span style={{ fontSize: 12, color: '#5B7A9D' }}>{open.length} of {tasks.length}</span>
        </div>
        {open.length === 0 && (
          <div style={{ background: '#FFFFFF', border: '1px solid #CCE0F7', borderRadius: 16, padding: 22, textAlign: 'center', color: '#5B7A9D', fontSize: 14 }}>
        ...
        <div style={{ marginTop: 32 }}>
          ...
          <div style={{ display: 'grid', gap: 8 }}>
            {data.kids.map(k => {
              ...
              return (
                <div key={k.id} style={{ background: '#FFFFFF', border: '1px solid #CCE0F7', borderLeft: '3px solid ' + (c?.hex || '#B8D2F0'), borderRadius: 14, padding: '14px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div>
                    <div style={{ fontFamily: displayFont, fontSize: 17, fontWeight: 500, letterSpacing: '-0.01em' }}>{k.name}</div>
                    <div style={{ fontSize: 12, color: '#5B7A9D', marginTop: 2 }}>{k.age ? `Age ${k.age} · ` : ''}{k.school || 'No school set'}</div>
                  </div>
                  {count > 0 && <span style={{ background: c?.tint || '#E4EEFF', color: c?.hex || '#1558CE', fontSize: 11, fontWeight: 600, padding: '4px 9px', borderRadius: 100, letterSpacing: '0.02em' }}>{count} open</span>}
                </div>
              );
            })}
          </div>
...
function TaskRow({ task, kidColour, onToggle }) {
  const accent = task.priority === 'high' ? '#1558CE' : task.priority === 'medium' ? '#C07818' : '#5B7A9D';
  return (
    <div style={{ background: '#FFFFFF', border: '1px solid #CCE0F7', borderRadius: 14, padding: '14px 16px', display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 8 }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 14.5, fontWeight: 500, lineHeight: 1.4, color: task.completed ? '#7A9ABE' : '#0C1A2E', textDecoration: task.completed ? 'line-through' : 'none' }}>{task.title}</div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginTop: 4, fontSize: 11.5, color: '#5B7A9D', flexWrap: 'wrap' }}>
          <span style={{ color: accent, fontWeight: 600 }}>{task.dueLabel}</span>
          {task.kidName && <>
            <span style={{ width: 3, height: 3, borderRadius: '50%', background: '#B8D2F0' }} />
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
              <span style={{ width: 7, height: 7, borderRadius: '50%', background: kidColour?.hex || '#5B7A9D' }} />
              {task.kidName}
            </span>
          </>}
          {task.source && <><span style={{ width: 3, height: 3, borderRadius: '50%', background: '#B8D2F0' }} />{task.source}</>}
        </div>
      </div>
    </div>
  );
...
function InboxView({ data, setData, displayFont }) {
  ...
  const processPaste = async () => {
    ...
    try {
      ...
      setError('Could not parse. Try again with clearer content.');
    }
  ...
  return (
    <div style={{ paddingTop: 24 }}>
      <div style={{ fontSize: 11, color: '#5B7A9D', letterSpacing: '0.16em', textTransform: 'uppercase', fontWeight: 500, marginBottom: 6 }}>Inbox Triage</div>
      ...
      <p style={{ fontSize: 14, color: '#3A567A', lineHeight: 1.5, margin: '0 0 20px', maxWidth: 480 }}>
      ...
      {!gmailConnected ? (
        <div style={{ background: 'linear-gradient(135deg, #FFFFFF 0%, #E4EEFF 100%)', border: '1px solid #C2D8F3', borderRadius: 18, padding: 22, marginBottom: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
            <GmailIcon />
            <span style={{ fontSize: 11, color: '#5B7A9D', letterSpacing: '0.14em', textTransform: 'uppercase', fontWeight: 600 }}>Continuous monitoring</span>
          </div>
          <h3 style={{ fontFamily: displayFont, fontSize: 22, fontWeight: 500, letterSpacing: '-0.02em', margin: '0 0 8px' }}>Connect your inbox</h3>
          <p style={{ fontSize: 13.5, color: '#3A567A', lineHeight: 1.55, margin: '0 0 14px' }}>
          ...
          <p style={{ fontSize: 11, color: '#7A9ABE', marginTop: 14, lineHeight: 1.5, fontStyle: 'italic' }}>Prototype: simulated Gmail with realistic sample emails for your kids. Real Gmail OAuth is a 1–2 week production build.</p>
        </div>
      ) : (
        <div style={{ background: '#FFFFFF', border: '1px solid #CCE0F7', borderRadius: 18, padding: 18, marginBottom: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14, gap: 10, flexWrap: 'wrap' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0, flex: 1 }}>
              <GmailIcon />
              <div style={{ minWidth: 0 }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: '#0C1A2E' }}>Gmail connected</div>
                <div style={{ fontSize: 11.5, color: '#5B7A9D', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{data.gmailAddress || 'parent@gmail.com'} · {untriaged.length} for triage</div>
              </div>
            </div>
            <button onClick={disconnectGmail} style={{ background: 'transparent', border: 'none', color: '#5B7A9D', fontSize: 11.5, cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600 }}>Disconnect</button>
          </div>
          ...
          {gmailEmails.length === 0 && (
            <div style={{ textAlign: 'center', padding: 20, color: '#5B7A9D', fontSize: 13 }}>No kid-related emails found. Inbox is quiet.</div>
          )}

          {gmailEmails.map(email => (
            <div key={email.id} style={{ borderTop: '1px solid #CCE0F7', padding: '12px 0' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3, flexWrap: 'wrap' }}>
                <span style={{ fontSize: 11.5, color: '#5B7A9D', fontWeight: 500 }}>{email.from.split('<')[0].trim()}</span>
                {email.kidName && <span style={{ background: '#E4EEFF', color: '#1558CE', fontSize: 10, fontWeight: 600, padding: '2px 6px', borderRadius: 6, letterSpacing: '0.02em' }}>{email.kidName}</span>}
                <span style={{ fontSize: 11, color: '#7A9ABE' }}>· {timeAgo(email.receivedAt)}</span>
              </div>
              <div style={{ fontSize: 14, fontWeight: 600, color: '#0C1A2E', lineHeight: 1.35, marginBottom: 3 }}>{email.subject}</div>
              <div style={{ fontSize: 12.5, color: '#4A6485', lineHeight: 1.45, overflow: 'hidden', textOverflow: 'ellipsis', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>
                {email.triaged && email.summary ? email.summary : email.preview}
              </div>
              <div style={{ marginTop: 8 }}>
                {email.triaged ? (
                  <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 11.5, color: '#163E7E', fontWeight: 600 }}>
                    <Check size={12} /> Triaged · {email.tasksAdded || 0} task{email.tasksAdded === 1 ? '' : 's'} added
                  </div>
                ) : (
                  <button onClick={() => triageOne(email)} disabled={triagingId === email.id || batchTriaging} style={{ background: 'transparent', border: '1px solid #B8D2F0', color: '#1558CE', borderRadius: 8, padding: '5px 10px', fontSize: 11.5, fontWeight: 600, cursor: (triagingId === email.id || batchTriaging) ? 'wait' : 'pointer', fontFamily: 'inherit', display: 'inline-flex', alignItems: 'center', gap: 5 }}>
      ...
      <details open={showPaste} onToggle={e => setShowPaste(e.currentTarget.open)} style={{ background: '#FFFFFF', border: '1px solid #CCE0F7', borderRadius: 14, padding: 14, marginBottom: 16 }}>
        <summary style={{ cursor: 'pointer', fontSize: 13, fontWeight: 600, color: '#0C1A2E', listStyle: 'none', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <textarea
          ...
          rows={6}
          style={{ width: '100%', boxSizing: 'border-box', padding: '12px 14px', borderRadius: 10, border: '1px solid #B8D2F0', background: '#F4F9FF', fontSize: 13.5, lineHeight: 1.5, fontFamily: 'inherit', color: '#0C1A2E', resize: 'vertical', outline: 'none', marginTop: 10 }}
        />
      ...
      {error && (
        <div style={{ marginTop: 12, padding: '10px 14px', background: '#FCEBE6', border: '1px solid #F0C8BD', borderRadius: 10, color: '#C0392B', fontSize: 13, display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      {(data.inbox || []).length > 0 && (
        <div style={{ marginTop: 8 }}>
          ...
          {(data.inbox || []).slice(0, 5).map(item => (
            <div key={item.id} style={{ background: '#FFFFFF', border: '1px solid #CCE0F7', borderRadius: 12, padding: '10px 14px', marginBottom: 6, fontSize: 12.5 }}>
              <div style={{ color: '#5B7A9D', fontSize: 11, marginBottom: 3 }}>{item.source}</div>
              <div style={{ color: '#0C1A2E', lineHeight: 1.4 }}>{item.summary}</div>
              <div style={{ color: '#1558CE', fontSize: 11, fontWeight: 600, marginTop: 3 }}>+{item.count} task{item.count === 1 ? '' : 's'}</div>
            </div>
          ))}
        </div>
      )}
...
function GmailIcon({ mini }) {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" style={{ flexShrink: 0 }}>
      <rect x="2" y="4" width="20" height="16" rx="2.5" fill="#FFFFFF" stroke="#B8D2F0" strokeWidth="1.2"/>
      <path d="M2.5 5.5 L12 13 L21.5 5.5" stroke="#1558CE" strokeWidth="1.5" fill="none" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  );
}
...
function MealsView({ data, setData, displayFont }) {
  ...
  const generatePlan = async () => {
    ...
    try {
      ...
      setError("Couldn't generate the plan. Try once more.");
    }
    setLoading(false);
  };
  ...
  return (
    <div style={{ paddingTop: 24 }}>
      <div style={{ fontSize: 11, color: '#5B7A9D', letterSpacing: '0.16em', textTransform: 'uppercase', fontWeight: 500, marginBottom: 6 }}>This Week's Menu</div>
      <h2 style={{ fontFamily: displayFont, fontSize: 30, fontWeight: 500, letterSpacing: '-0.025em', lineHeight: 1.05, margin: '0 0 8px' }}>
      ...
      {!plan && (
        <div style={{ background: '#FFFFFF', border: '1px solid #CCE0F7', borderRadius: 18, padding: 28, textAlign: 'center' }}>
          <div style={{ fontFamily: displayFont, fontSize: 22, fontWeight: 500, letterSpacing: '-0.02em', marginBottom: 8, color: '#0C1A2E' }}>No plan yet.</div>
          <p style={{ fontSize: 14, color: '#5B7A9D', lineHeight: 1.5, margin: '0 0 18px' }}>Let me draft one based on what I know about your kids.</p>
          ...
          {data.kids.length === 0 && <p style={{ fontSize: 12, color: '#1558CE', marginTop: 12 }}>Add kids first from the Settings menu.</p>}
        </div>
      )}
      ...
      {plan && plan.days && (
        <>
          ...
          <div style={{ display: 'flex', gap: 6, overflowX: 'auto', paddingBottom: 12, marginBottom: 4, marginLeft: -20, paddingLeft: 20, paddingRight: 20 }}>
            {plan.days.map((d, i) => (
              <button key={i} onClick={() => setActiveDay(i)} style={{ background: activeDay === i ? '#0C1A2E' : '#FFFFFF', color: activeDay === i ? '#F4F9FF' : '#0C1A2E', border: `1px solid ${activeDay === i ? '#0C1A2E' : '#CCE0F7'}`, borderRadius: 12, padding: '10px 14px', fontFamily: 'inherit', fontSize: 12, fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap', letterSpacing: '0.02em' }}>
                {d.day.slice(0, 3)}
              </button>
            ))}
          </div>
          ...
          {plan.days[activeDay] && (() => {
            ...
            const appealColor = d.kidAppeal === 'high' ? '#163E7E' : d.kidAppeal === 'medium' ? '#C07818' : '#1558CE';
            return (
              <div style={{ background: '#FFFFFF', border: '1px solid #CCE0F7', borderRadius: 18, padding: 22, marginBottom: 20 }}>
                <div style={{ fontSize: 11, letterSpacing: '0.16em', textTransform: 'uppercase', color: '#5B7A9D', fontWeight: 600, marginBottom: 8 }}>{d.day}</div>
                <h3 style={{ fontFamily: displayFont, fontSize: 26, fontWeight: 500, letterSpacing: '-0.025em', lineHeight: 1.1, margin: '0 0 10px', color: '#0C1A2E' }}>{d.name}</h3>
                <p style={{ fontSize: 14.5, lineHeight: 1.55, color: '#3A567A', margin: '0 0 16px' }}>{d.description}</p>
                <div style={{ display: 'flex', gap: 14, fontSize: 12, color: '#4A6485', flexWrap: 'wrap' }}>
                  <span><strong style={{ color: '#0C1A2E' }}>{d.prepTime}</strong> min prep</span>
          ...
          {plan.shoppingList && plan.shoppingList.length > 0 && (
            <div style={{ background: '#E4EEFF', border: '1px solid #C2D8F3', borderRadius: 18, padding: 22, marginBottom: 16 }}>
              ...
              <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: '6px 12px' }}>
                {plan.shoppingList.map((item, i) => (
                  <li key={i} style={{ fontSize: 13.5, color: '#0C1A2E', display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ width: 4, height: 4, background: '#1558CE', borderRadius: '50%', display: 'inline-block' }} />
                    {item}
                  </li>
                ))}
              </ul>
...
function CalendarView({ data, setData, setView, displayFont }) {
  ...
  const handleExport = () => {
    ...
    const ics = generateICS(upcoming, (data.familyName || 'Family') + ' — Nestly');
    const date = new Date().toISOString().slice(0, 10);
    downloadICS(ics, 'nestly-' + date + '.ics');
    setExportDone(true);
  ...
  return (
    <div style={{ paddingTop: 24 }}>
      <div style={{ fontSize: 11, color: '#5B7A9D', letterSpacing: '0.16em', textTransform: 'uppercase', fontWeight: 500, marginBottom: 6 }}>The Family Calendar</div>
      <h2 style={{ fontFamily: displayFont, fontSize: 30, fontWeight: 500, letterSpacing: '-0.025em', lineHeight: 1.05, margin: '0 0 8px' }}>
        Everyone's dates. <em>One place.</em>
      </h2>
      <p style={{ fontSize: 14, color: '#3A567A', lineHeight: 1.5, margin: '0 0 18px', maxWidth: 480 }}>
      ...
      {exportDone && (
        <div style={{ background: '#E3EFFF', border: '1px solid #9EC4EE', borderRadius: 10, padding: '10px 14px', marginBottom: 12, color: '#163E7E', fontSize: 12.5, lineHeight: 1.45 }}>
      ...
      <button onClick={() => setView('holidays')} style={{ background: 'linear-gradient(135deg, #EBF2FF 0%, #D6E9FF 100%)', border: '1px solid #AACBE8', borderRadius: 14, padding: '12px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', cursor: 'pointer', fontFamily: 'inherit', marginBottom: 20, gap: 8 }}>
        <div style={{ textAlign: 'left', minWidth: 0 }}>
          <div style={{ fontSize: 11, color: '#5B7A9D', letterSpacing: '0.06em', textTransform: 'uppercase', fontWeight: 600, marginBottom: 2 }}>Winter break · 6–17 July</div>
          <div style={{ fontFamily: displayFont, fontSize: 16, fontWeight: 500, color: '#0C1A2E', letterSpacing: '-0.01em' }}>Find programs for the next school holidays</div>
        </div>
        <ChevronRight size={18} color="#1558CE" style={{ flexShrink: 0 }} />
      </button>
      ...
      {totalCount === 0 && (
        <div style={{ background: '#FFFFFF', border: '1px solid #CCE0F7', borderRadius: 16, padding: 24, textAlign: 'center', marginTop: 16 }}>
          ...
          <p style={{ fontSize: 13.5, color: '#5B7A9D', lineHeight: 1.5, margin: 0 }}>Add an event, or let the inbox triage and holiday search fill it for you.</p>
        </div>
      )}

      <p style={{ fontSize: 11, color: '#7A9ABE', marginTop: 20, lineHeight: 1.5, fontStyle: 'italic', textAlign: 'center' }}>
...
function Section({ title, events, onEdit, displayFont }) {
  ...
  return (
    <div style={{ marginTop: 18 }}>
      <h3 style={{ fontFamily: displayFont, fontSize: 13, fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase', color: '#5B7A9D', margin: '0 0 10px' }}>{title}</h3>
...
function EventRow({ event, onEdit }) {
  const kindColor = {
    ...
    'birthday': '#A8538C',
    'appointment': '#1A4EA8',
    'holiday-program': '#163E7E',
    'school-holiday': '#C07818',
    'public-holiday': '#4A6485',
    'task': '#1558CE'
  };
  const kindLabel = {
    'event': 'Event',
  ...
  const c = kindColor[event.kind] || '#4A6485';
    ...
    <div onClick={() => editable && onEdit && onEdit(event)} style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3, flexWrap: 'wrap' }}>
          ...
          {event.kidName && <span style={{ background: event.kidColour?.tint || '#E4EEFF', color: event.kidColour?.hex || '#1558CE', fontSize: 10, fontWeight: 600, padding: '2px 6px', borderRadius: 5, letterSpacing: '0.02em' }}>{event.kidName}</span>}
        </div>
        <div style={{ fontSize: 14.5, fontWeight: 600, color: '#0C1A2E', lineHeight: 1.35, marginBottom: 4 }}>{event.title}</div>
        <div style={{ fontSize: 12, color: '#4A6485', display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <span style={{ fontWeight: 600 }}>{dateText}</span>
          {timeText && <span>· {timeText}</span>}
        </div>
        {event.notes && <div style={{ fontSize: 12, color: '#5B7A9D', marginTop: 6, lineHeight: 1.45 }}>{event.notes}</div>}
      </div>
      {editable && <Edit2 size={14} style={{ color: '#7A9ABE', flexShrink: 0, marginTop: 2 }} />}
    </div>
  );
}
...
function AddEventModal({ kids, initial, onSave, onDelete, onClose, displayFont }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(26,22,18,0.4)', zIndex: 60, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: '#F4F9FF', borderRadius: '20px 20px 0 0', width: '100%', maxWidth: 640, maxHeight: '90vh', overflow: 'auto', padding: '24px 24px 40px', boxShadow: '0 -8px 40px rgba(0,0,0,0.15)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
          ...
          <button onClick={onClose} style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: '#4A6485' }}><X size={20} /></button>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 6, marginBottom: 14 }}>
          {[
            ...
            <button key={k.id} onClick={() => setKind(k.id)} style={{
              background: kind === k.id ? '#0C1A2E' : '#FFFFFF',
              color: kind === k.id ? '#F4F9FF' : '#0C1A2E',
              border: '1px solid ' + (kind === k.id ? '#0C1A2E' : '#B8D2F0'),
              borderRadius: 10,
              padding: '8px 10px',
              fontSize: 12.5,
              fontWeight: 600,
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          ...
          <div>
            <label style={labelStyle}>End date <span style={{ color: '#7A9ABE', textTransform: 'none', letterSpacing: 0, fontWeight: 400 }}>· opt</span></label>
            <input type="date" value={endDate} onChange={e => setEndDate(e.target.value)} style={inputStyle} />
          </div>
          <div>
            <label style={labelStyle}>Start time <span style={{ color: '#7A9ABE', textTransform: 'none', letterSpacing: 0, fontWeight: 400 }}>· opt</span></label>
            <input type="time" value={startTime} onChange={e => setStartTime(e.target.value)} style={inputStyle} />
          </div>
          <div>
            <label style={labelStyle}>End time <span style={{ color: '#7A9ABE', textTransform: 'none', letterSpacing: 0, fontWeight: 400 }}>· opt</span></label>
            <input type="time" value={endTime} onChange={e => setEndTime(e.target.value)} style={inputStyle} />
          </div>
        </div>

        <label style={{ ...labelStyle, marginTop: 4 }}>For which child <span style={{ color: '#7A9ABE', textTransform: 'none', letterSpacing: 0, fontWeight: 400 }}>· optional</span></label>
        ...
        <label style={labelStyle}>Notes <span style={{ color: '#7A9ABE', textTransform: 'none', letterSpacing: 0, fontWeight: 400 }}>· optional</span></label>
        ...
        {onDelete && (
          <div style={{ marginTop: 20, paddingTop: 18, borderTop: '1px solid #CCE0F7' }}>
            {!confirmingDelete ? (
              <button onClick={() => setConfirmingDelete(true)} style={{ background: 'transparent', border: 'none', color: '#C0392B', fontSize: 12.5, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', display: 'inline-flex', alignItems: 'center', gap: 5 }}>
              ...
              <div style={{ padding: 10, background: '#FCEBE6', border: '1px solid #F0C8BD', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, flexWrap: 'wrap' }}>
                <span style={{ fontSize: 12.5, color: '#C0392B' }}>Delete event?</span>
                <div style={{ display: 'flex', gap: 6 }}>
                  <button onClick={() => setConfirmingDelete(false)} style={{ background: 'transparent', border: '1px solid #B8D2F0', color: '#0C1A2E', borderRadius: 8, padding: '5px 10px', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>Cancel</button>
                  <button onClick={onDelete} style={{ background: '#C0392B', color: '#FFFFFF', border: 'none', borderRadius: 8, padding: '5px 10px', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>Delete</button>
                </div>
              </div>
            )}
          </div>
...
Generate 6-8 specific holiday program recommendations realistic for this suburb during these dates. Mix categories. Best fit first.`;
      ...
      setError('Search failed. Try a clearer suburb name or fewer dates.');
    }
    setSearching(false);
  };
  ...
  const typeColor = (type) => {
    const map = { sports: '#163E7E', stem: '#1A4EA8', arts: '#1558CE', outdoor: '#1E7A5A', academic: '#0E5C96', mixed: '#4A6485' };
    return map[type] || '#4A6485';
  };
  ...
  return (
    <div style={{ paddingTop: 24 }}>
      <div style={{ fontSize: 11, color: '#5B7A9D', letterSpacing: '0.16em', textTransform: 'uppercase', fontWeight: 500, marginBottom: 6 }}>School Holidays</div>
      <h2 style={{ fontFamily: displayFont, fontSize: 30, fontWeight: 500, letterSpacing: '-0.025em', lineHeight: 1.05, margin: '0 0 8px' }}>
      ...
      {/* Search form */}
      <div style={{ background: '#FFFFFF', border: '1px solid #CCE0F7', borderRadius: 18, padding: 20, marginBottom: 16 }}>
        ...
        {data.kids.length > 0 ? (
          ...
          <div style={{ padding: '12px 14px', background: '#E4EEFF', borderRadius: 10, fontSize: 13, color: '#1558CE', marginBottom: 8 }}>Add a child in Settings first.</div>
        )}
        ...
        <div style={{ position: 'relative' }}>
          <MapPin size={14} style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: '#5B7A9D' }} />
          <input value={location} onChange={e => setLocation(e.target.value)} placeholder="e.g. Tallawong, NSW" style={{ ...inputStyle, paddingLeft: 32 }} />
        </div>

        <label style={labelStyle}>Budget per week <span style={{ color: '#7A9ABE', textTransform: 'none', letterSpacing: 0, fontWeight: 400 }}>· optional</span></label>
      ...
      {error && (
        <div style={{ marginBottom: 12, padding: '10px 14px', background: '#FCEBE6', border: '1px solid #F0C8BD', borderRadius: 10, color: '#C0392B', fontSize: 13, display: 'flex', alignItems: 'center', gap: 8 }}>
          <AlertCircle size={14} /> {error}
        </div>
      )}
      ...
      {savedPrograms.length > 0 && (
        <div style={{ background: 'linear-gradient(135deg, #163E7E 0%, #0D2B6A 100%)', borderRadius: 14, padding: '14px 16px', marginBottom: 12, color: '#EDF4FF' }}>
      ...
      {results.length > 0 && (
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 12, marginTop: 16 }}>
            ...
            {selectedKid && <span style={{ fontSize: 12, color: '#5B7A9D' }}>for {selectedKid.name}</span>}
          </div>

          {results.map(p => {
            const isSaved = saved.includes(p.id);
            return (
              <div key={p.id} style={{ background: '#FFFFFF', border: '1px solid #CCE0F7', borderRadius: 16, padding: 16, marginBottom: 10 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8, flexWrap: 'wrap' }}>
                  ...
                  <span style={{ fontSize: 11.5, color: '#5B7A9D' }}>{p.ageRange}</span>
                  {p.distanceKm !== undefined && p.distanceKm !== null && <span style={{ fontSize: 11.5, color: '#5B7A9D' }}>· {p.distanceKm}km</span>}
                </div>
                <h4 style={{ fontFamily: displayFont, fontSize: 19, fontWeight: 500, letterSpacing: '-0.015em', margin: '0 0 4px', color: '#0C1A2E', lineHeight: 1.2 }}>{p.name}</h4>
                <div style={{ fontSize: 12.5, color: '#4A6485', marginBottom: 10 }}>{p.provider}</div>
                <p style={{ fontSize: 13.5, lineHeight: 1.5, color: '#253C58', margin: '0 0 12px' }}>{p.fitReason}</p>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px 12px', fontSize: 11.5, color: '#5B7A9D', marginBottom: 12, paddingTop: 10, borderTop: '1px solid #CCE0F7' }}>
                  <div><span style={{ color: '#0C1A2E', fontWeight: 700 }}>${p.costPerWeek}</span> / week</div>
                  <div style={{ textAlign: 'right' }}>{p.schedule}</div>
                  <div style={{ gridColumn: 'span 2' }}>{p.weeks}</div>
                  {p.bookingHint && <div style={{ gridColumn: 'span 2', color: '#7A9ABE', fontStyle: 'italic', marginTop: 2 }}>{p.bookingHint}</div>}
                </div>
                <button onClick={() => toggleSave(p.id)} style={{ background: isSaved ? '#163E7E' : 'transparent', color: isSaved ? '#F4F9FF' : '#0C1A2E', border: `1px solid ${isSaved ? '#163E7E' : '#B8D2F0'}`, borderRadius: 100, padding: '7px 14px', fontSize: 12.5, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', display: 'inline-flex', alignItems: 'center', gap: 6 }}>
          ...
          <p style={{ fontSize: 11, color: '#7A9ABE', marginTop: 14, lineHeight: 1.5, fontStyle: 'italic', textAlign: 'center', padding: '12px 16px', background: '#E4EEFF', borderRadius: 10 }}>
...
function AssistantView({ data, setData, displayFont }) {
  ...
  const send = async (text) => {
    ...
    const system = `You are the Nestly AI assistant for ${data.familyName}. You speak with the warmth of a trusted friend who's seen it all and the precision of an excellent operator. You are concise, dry, and never sycophantic. You never use emojis or exclamation marks. Address the parent directly. Two-to-four sentences typically; longer only when listing concrete options.
...
- This week's meal plan: ${data.mealPlan ? data.mealPlan.days.map(d => `${d.day}: ${d.name}`).join('; ') : 'none'}`;
    ...
    try {
      ...
      setMessages([...newMessages, { role: 'assistant', content: "I couldn't reach the line just now. One more time?" }]);
    }
    setLoading(false);
  };
  ...
  return (
    <div style={{ paddingTop: 24, display: 'flex', flexDirection: 'column', minHeight: 'calc(100vh - 200px)' }}>
      <div style={{ fontSize: 11, color: '#5B7A9D', letterSpacing: '0.16em', textTransform: 'uppercase', fontWeight: 500, marginBottom: 6 }}>Your AI Assistant</div>
      ...
      <div ref={scrollRef} style={{ flex: 1, marginBottom: 12, minHeight: 220 }}>
        {messages.length === 0 && (
          <div>
            <p style={{ fontSize: 13.5, color: '#5B7A9D', lineHeight: 1.55, margin: '0 0 14px' }}>I know your family. Try one of these to start:</p>
            <div style={{ display: 'grid', gap: 6 }}>
              {suggestions.map((s, i) => (
                <button key={i} onClick={() => send(s)} style={{ background: '#FFFFFF', border: '1px solid #CCE0F7', borderRadius: 12, padding: '12px 14px', textAlign: 'left', fontSize: 14, fontFamily: 'inherit', color: '#0C1A2E', cursor: 'pointer', lineHeight: 1.4, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
                  <span>{s}</span>
                  <ChevronRight size={14} color="#1558CE" />
                </button>
              ))}
            </div>
          </div>
        ...
        {messages.map((m, i) => (
          <div key={i} style={{ display: 'flex', justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start', marginBottom: 10 }}>
            <div style={{
              ...
              padding: '11px 15px',
              borderRadius: 16,
              background: m.role === 'user' ? '#0C1A2E' : '#FFFFFF',
              color: m.role === 'user' ? '#F4F9FF' : '#0C1A2E',
              border: m.role === 'user' ? 'none' : '1px solid #CCE0F7',
              fontSize: 14.5,
              lineHeight: 1.55,
        ...
        {loading && (
          <div style={{ display: 'flex', justifyContent: 'flex-start' }}>
            <div style={{ padding: '12px 15px', borderRadius: 16, background: '#FFFFFF', border: '1px solid #CCE0F7', color: '#5B7A9D', fontSize: 13, fontStyle: 'italic' }}>thinking…</div>
          </div>
        )}
      </div>

      <div style={{ position: 'sticky', bottom: 88, background: '#F4F9FF', paddingTop: 8 }}>
        <div style={{ display: 'flex', gap: 8 }}>
          <input
            ...
            style={{ flex: 1, padding: '12px 16px', borderRadius: 14, border: '1px solid #B8D2F0', background: '#FFFFFF', fontSize: 14.5, fontFamily: 'inherit', color: '#0C1A2E', outline: 'none' }}
          />
          <button onClick={() => send()} disabled={loading || !input.trim()} style={{ background: '#0C1A2E', color: '#F4F9FF', border: 'none', borderRadius: 14, padding: '0 16px', cursor: loading || !input.trim() ? 'not-allowed' : 'pointer', opacity: loading || !input.trim() ? 0.4 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Send size={16} />
          </button>
        </div>
        {messages.length > 0 && (
          <button onClick={() => { setMessages([]); setData({ ...data, chats: [] }); }} style={{ background: 'transparent', border: 'none', color: '#5B7A9D', fontSize: 11.5, marginTop: 6, cursor: 'pointer', fontFamily: 'inherit' }}>Clear conversation</button>
        )}
      </div>
    </div>
  );
...
// SETTINGS PANEL
// =====================================================================
function SettingsPanel({ data, setData, onClose, displayFont }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(26,22,18,0.4)', zIndex: 50, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: '#F4F9FF', borderRadius: '20px 20px 0 0', width: '100%', maxWidth: 640, maxHeight: '85vh', overflow: 'auto', padding: '24px 24px 40px', boxShadow: '0 -8px 40px rgba(0,0,0,0.15)' }}>
        ...
        <div style={{ marginTop: 32, paddingTop: 20, borderTop: '1px solid #CCE0F7' }}>
          {!confirmingReset ? (
            <button onClick={() => setConfirmingReset(true)} style={{ background: 'transparent', border: '1px solid #FBBFBF', color: '#C0392B', borderRadius: 100, padding: '9px 16px', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
              Reset all data
            </button>
          ) : (
            <div style={{ padding: 12, background: '#FCEBE6', border: '1px solid #F0C8BD', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, flexWrap: 'wrap' }}>
              <span style={{ fontSize: 13, color: '#C0392B' }}>Clear all data permanently?</span>
              <div style={{ display: 'flex', gap: 6 }}>
                <button onClick={() => setConfirmingReset(false)} style={{ background: 'transparent', border: '1px solid #B8D2F0', color: '#0C1A2E', borderRadius: 8, padding: '6px 12px', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>Cancel</button>
                <button onClick={reset} style={{ background: '#C0392B', color: '#FFFFFF', border: 'none', borderRadius: 8, padding: '6px 12px', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>Reset</button>
              </div>
            </div>
          )}
          <p style={{ fontSize: 11.5, color: '#5B7A9D', marginTop: 14, lineHeight: 1.5 }}>Nestly · v0.2 · All data stays on this device.</p>
        </div>
      </div>
    </div>
  );
...
function KidEditor({ kid, onChange, onRemove }) {
  ...
  return (
    <div style={{ background: '#FFFFFF', border: '1px solid #CCE0F7', borderLeft: '3px solid ' + currentColour.hex, borderRadius: 14, padding: 14, marginBottom: 8 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: expanded ? 12 : 0 }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: '#5B7A9D' }}>Age {kid.age || '?'} · {kid.school || 'no school'}</div>
        </div>
        <button onClick={() => { setExpanded(!expanded); setConfirmingRemove(false); }} style={{ background: 'transparent', border: 'none', color: '#1558CE', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>{expanded ? 'Done' : 'Edit'}</button>
      </div>
      {expanded && (
        <>
          <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 4 }}>
            {KID_COLOURS.map(c => (
              <button key={c.id} onClick={() => onChange({ ...kid, colour: c.id })} title={c.name} style={{
                ...
                background: c.hex,
                border: kid.colour === c.id ? '3px solid #0C1A2E' : '2px solid transparent',
                cursor: 'pointer',
                padding: 0,
                outlineOffset: 2
              }} />
          ...
          {!confirmingRemove ? (
            <button onClick={() => setConfirmingRemove(true)} style={{ background: 'transparent', border: 'none', color: '#C0392B', fontSize: 12, fontWeight: 600, cursor: 'pointer', marginTop: 12, display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            <div style={{ marginTop: 12, padding: 10, background: '#FCEBE6', border: '1px solid #F0C8BD', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, flexWrap: 'wrap' }}>
              <span style={{ fontSize: 12.5, color: '#C0392B' }}>Remove {kid.name || 'this child'}?</span>
              <div style={{ display: 'flex', gap: 6 }}>
                <button onClick={() => setConfirmingRemove(false)} style={{ background: 'transparent', border: '1px solid #B8D2F0', color: '#0C1A2E', borderRadius: 8, padding: '5px 10px', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>Cancel</button>
                <button onClick={onRemove} style={{ background: '#C0392B', color: '#FFFFFF', border: 'none', borderRadius: 8, padding: '5px 10px', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>Remove</button>
              </div>
            </div>
          )}
        </>
