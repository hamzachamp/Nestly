    ...
    <div style={{ paddingTop: 24 }}>
      ...
      {!gmailConnected ? (
        <div style={{ background: 'linear-gradient(135deg, #FFFFFF 0%, #F1E9D8 100%)', border: '1px solid #E0D3BC', borderRadius: 18, padding: 22, marginBottom: 20 }}>
          ...
          <p style={{ fontSize: 11, color: '#A89E92', marginTop: 14, lineHeight: 1.5, fontStyle: 'italic' }}>{realGmailMode ? 'Real Gmail connected. We fetch up to 20 recent emails mentioning your kids\u2019 names. Access tokens last 1 hour \u2014 click Refresh anytime to re-sync.' : 'Prototype: simulated Gmail. Set VITE_GOOGLE_CLIENT_ID in .env to enable real Gmail OAuth.'}</p>
        </div>
      ) : (
        <div style={{ background: '#FFFFFF', border: '1px solid #E8E0D3', borderRadius: 18, padding: 18, marginBottom: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14, gap: 10, flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              {realGmailMode && (
                <button onClick={refreshGmail} disabled={syncing} style={{ background: 'transparent', border: '1px solid #D8CFC0', color: '#8B3A2C', borderRadius: 8, padding: '5px 10px', fontSize: 11.5, fontWeight: 600, cursor: syncing ? 'wait' : 'pointer', fontFamily: 'inherit', display: 'inline-flex', alignItems: 'center', gap: 5 }}>
          ...
          {gmailEmails.map(email => (
            <div key={email.id} style={{ borderTop: '1px solid #F0E8D8', padding: '12px 0' }}>
              ...
              <div style={{ marginTop: 8 }}>
                {email.triaged ? (
                  <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 11.5, color: '#2D5240', fontWeight: 600, flexWrap: 'wrap' }}>
        ...
        <div style={{ marginTop: 8 }}>
          ...
          {(data.inbox || []).slice(0, 5).map(item => (
            <div key={item.id} style={{ background: '#FFFFFF', border: '1px solid #E8E0D3', borderRadius: 12, padding: '10px 14px', marginBottom: 6, fontSize: 12.5 }}>
              ...
              <div style={{ color: '#8B3A2C', fontSize: 11, fontWeight: 600, marginTop: 3 }}>+{item.count} task{item.count === 1 ? '' : 's'}{item.eventCount ? `, +${item.eventCount} event${item.eventCount === 1 ? '' : 's'}` : ''}</div>
            </div>
          ))}
        </div>
      )}
...
function CalendarView({ data, setData, setView, displayFont }) {
  ...
  return (
    <div style={{ paddingTop: 24 }}>
      ...
      <div style={{ display: 'inline-flex', background: '#FFFFFF', border: '1px solid #E8E0D3', borderRadius: 10, padding: 3, marginBottom: 14 }}>
        <button onClick={() => setViewMode('list')} style={{ background: viewMode === 'list' ? '#8B3A2C' : 'transparent', color: viewMode === 'list' ? '#FFFFFF' : '#5C534A', border: 'none', borderRadius: 7, padding: '6px 14px', fontSize: 12.5, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>List</button>
        <button onClick={() => setViewMode('month')} style={{ background: viewMode === 'month' ? '#8B3A2C' : 'transparent', color: viewMode === 'month' ? '#FFFFFF' : '#5C534A', border: 'none', borderRadius: 7, padding: '6px 14px', fontSize: 12.5, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>Month</button>
      </div>
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        <button onClick={goPrev} style={{ background: '#FFFFFF', border: '1px solid #E8E0D3', borderRadius: 8, width: 32, height: 32, cursor: 'pointer', fontSize: 18, color: '#5C534A', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0 }}>‹</button>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <h3 style={{ fontFamily: displayFont, fontSize: 18, fontWeight: 500, letterSpacing: '-0.01em', margin: 0 }}>{monthName}</h3>
          <button onClick={goToday} style={{ background: 'transparent', border: '1px solid #D8CFC0', color: '#8B3A2C', borderRadius: 6, padding: '3px 9px', fontSize: 11, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>Today</button>
        </div>
        <button onClick={goNext} style={{ background: '#FFFFFF', border: '1px solid #E8E0D3', borderRadius: 8, width: 32, height: 32, cursor: 'pointer', fontSize: 18, color: '#5C534A', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0 }}>›</button>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 2, background: '#FFFFFF', border: '1px solid #E8E0D3', borderRadius: 12, padding: 6 }}>
        {['M','T','W','T','F','S','S'].map((d, i) => (
          <div key={i} style={{ fontSize: 10, fontWeight: 700, color: '#8B7E70', textAlign: 'center', padding: '4px 0', letterSpacing: '0.08em' }}>{d}</div>
        ))}
        {cells.map((d, i) => {
          const iso = d.toISOString().slice(0, 10);
          ...
          return (
            <button
              ...
              style={{
                background: isSelected ? '#8B3A2C' : isToday ? '#F1E9D8' : 'transparent',
                color: isSelected ? '#FFFFFF' : isOtherMonth ? '#C8BFB1' : '#1A1612',
                border: 'none',
                borderRadius: 6,
                padding: '6px 2px 4px',
                minHeight: 44,
      ...
      {selectedDay && (
        <div style={{ marginTop: 14 }}>
          <h3 style={{ fontFamily: displayFont, fontSize: 14, fontWeight: 600, letterSpacing: '0.04em', textTransform: 'uppercase', color: '#8B7E70', margin: '0 0 8px' }}>
            {new Date(selectedDay + 'T00:00:00').toLocaleDateString('en-AU', { weekday: 'long', day: 'numeric', month: 'long' })}
          </h3>
          {selectedEvents.length === 0 ? (
            <div style={{ background: '#FFFFFF', border: '1px solid #E8E0D3', borderRadius: 10, padding: '14px 16px', fontSize: 13, color: '#8B7E70', fontStyle: 'italic' }}>Nothing scheduled.</div>
          ) : (
      ...
      {!selectedDay && (
        <p style={{ fontSize: 12, color: '#A89E92', marginTop: 12, lineHeight: 1.5, textAlign: 'center', fontStyle: 'italic' }}>
